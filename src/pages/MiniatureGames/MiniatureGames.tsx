...
// A single miniature game from the local dataset. Derived from the
// dataset so the type stays accurate if the data shape changes.
type MiniatureGame = (typeof miniatureGames)[number]
 ...
 * Miniature Games
 * --------------
 * Browse and replay a curated set of miniature chess games.
 *
...
function MiniatureGames() {
  // The currently-selected game index in the Miniature Games dataset.
  ...
  const selectedGame: MiniatureGame | undefined = miniatureGames[selectedEventIndex]
  ...
  useEffect(() => {
    if (miniatureGames.length === 0) return

    const randomIndex = getRandomNumber(miniatureGames.length)
  ...
  useEffect(() => {
    document.title = 'Miniature Games'
  }, [])
  ...
  return (
    <div className="page-container">
      <h1 className="heading-1">Miniature Games</h1>
